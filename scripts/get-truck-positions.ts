...
export async function getTruckPositions(key: string) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error(
        "Failed to fetch truck positions:",
...
if (require.main === module) {
  ...
  if (args.length !== 1) {
    console.error("Usage: ts-node get-truck-positions.ts <key>");
    process.exit(1);
  }

  const [key] = args;

  getTruckPositions(key).catch((error) => {
